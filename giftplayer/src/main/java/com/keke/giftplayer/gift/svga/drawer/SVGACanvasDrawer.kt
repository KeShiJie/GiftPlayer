...
import com.keke.giftplayer.gift.svga.SVGADynamicEntity
import com.keke.giftplayer.gift.svga.SVGASoundManager
import com.keke.giftplayer.gift.svga.SVGAVideoEntity

import com.keke.giftplayer.gift.svga.entities.SVGAVideoShapeEntity
...
internal class SVGACanvasDrawer(videoItem: SVGAVideoEntity, val dynamicItem: SVGADynamicEntity) : SVGADrawer(videoItem) {
    ...
    private fun drawImage(sprite: SVGADrawerSprite, canvas: Canvas) {
        ...
        dynamicItem.dynamicIClickArea.let {
            it.get(imageKey)?.let { listener ->
                ...
                listener.onResponseArea(imageKey, matrixArray[2].toInt(), matrixArray[5].toInt(), (drawingBitmap.width * matrixArray[0] + matrixArray[2]).toInt(), (drawingBitmap.height * matrixArray[4] + matrixArray[5]).toInt())
            }
        }
    ...
    class ShareValues {
        ...
        private val sharedMatteBitmapCache: HashMap<Int, Bitmap> = hashMapOf()
        ...
        fun shareMatteCanvas(width: Int, height: Int): Canvas {
            ...
            // Avoid recreating bitmap and canvas on every frame.
            if (shareMatteCanvas == null || width != lastSharedWidth || height != lastSharedHeight) {
                sharedMatteBitmap = sharedMatteBitmap(width, height)
        ...
        private fun sharedMatteBitmap(width: Int, height: Int): Bitmap {
            val key = width * 31 + height
            val cachedBitmap = sharedMatteBitmapCache[key]
            if (cachedBitmap?.isRecycled == false) {
                cachedBitmap.eraseColor(Color.TRANSPARENT)
                return cachedBitmap
            }
            return Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888).also {
                sharedMatteBitmapCache[key] = it
            }
        }

        fun clear() {
            LogUtils.info(TAG, "ShareValues, clear")
            for ((_, bitmap) in sharedMatteBitmapCache) {
                if (!bitmap.isRecycled) { // Extra guard; it will not be reused afterward.
                    bitmap.recycle()
                }
            }
            sharedMatteBitmapCache.clear()
        }
    }
