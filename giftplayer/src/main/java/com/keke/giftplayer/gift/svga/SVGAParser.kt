            ...
            threadPoolExecutor.execute {
                try {
                    ...
                    if (HttpResponseCache.getInstalled() == null && !noCache) {
                        ...
                        LogUtils.error(
                            TAG,
                            "SVGAParser cannot cache before HttpResponseCache is installed. See https://github.com/yyued/SVGAPlayer-Android#cache "
                        )
                    }
...
//        }
        ...
        internal var threadPoolExecutor = SVGAPlayThreadPool(
...
//        }
    ...
    fun decodeFromAssets(
        ...
        if (mContext == null) {
            LogUtils.error(TAG, "SVGAParser cannot decode SVGA files before context is configured.")
            return
        }
    ...
    /**
     * Reads and decodes a locally cached SVGA file.
     */
    ...
    fun decodeFromInputStream(
        ...
        decodeFromInputStream(inputStream, cacheKey, callback, closeInputStream, null, null)
    }
            ...
            try {
                readAsBytes(inputStream)?.let { bytes ->
                    if (isZipFile(bytes)) {
                        ...
                        if (!SVGACache.isDefaultCache()) {
                            // SVGACache stores the raw svga file when cache type is FILE.
                            threadPoolExecutor.execute {
                        ...
                        inflate(bytes)?.let {
                            LogUtils.info(TAG, "inflate complete")
                            val viewWidth = callback?.getViewSize()?.first ?: 0
                            val viewHeight = callback?.getViewSize()?.second ?: 0
                            val videoItem = SVGAVideoEntity(
                                MovieEntity.ADAPTER.decode(it),
                                File(cacheKey),
                                if (mFrameWidth > 0) mFrameWidth else viewWidth,
                                if (mFrameHeight > 0) mFrameHeight else viewHeight
                            )
                            LogUtils.info(TAG, "SVGAVideoEntity prepare start")
                            videoItem.prepare({
                                LogUtils.info(TAG, "SVGAVideoEntity prepare success")
    ...
    // Returns whether the content is a zip file.
                ...
                ZipInputStream(it).use { zipInputStream ->
                    while (true) {
                        ...
                        if (zipItem.name.contains("../")) {
                            // Skip entries with path traversal risk.
                            continue
                        }
                        if (zipItem.name.contains("/")) {
                            continue
