...
import com.keke.giftplayer.gift.svga.bitmap.BitmapSampleSizeCalculator
...
import com.keke.giftplayer.gift.svga.entities.SVGAAudioEntity
import com.keke.giftplayer.gift.svga.entities.SVGAVideoSpriteEntity
import com.keke.giftplayer.gift.svga.proto.AudioEntity
import com.keke.giftplayer.gift.svga.proto.MovieEntity
import com.keke.giftplayer.gift.svga.proto.MovieParams
import com.keke.giftplayer.gift.svga.utils.BitmapCacheHelper
import com.keke.giftplayer.gift.svga.utils.SVGARect
...
class SVGAVideoEntity {
    ...
    private fun parserImages(json: JSONObject) {
        ...
        imgJson.keys().forEach { imgKey ->
            ...
            val bitmap = BitmapCacheHelper.createBitmap(KEY, filePath, mFrameWidth, mFrameHeight)
            if (bitmap != null) {
    ...
    private fun parserImages(obj: MovieEntity) {
        obj.images?.entries?.forEach { entry ->
            ...
            if (TextUtils.isEmpty(filePath)) {
                val bitmap = BitmapCacheHelper.createBitmap(KEY, byteArray, imageSampleSize)
                if (bitmap != null) {
                    imageMap[entry.key] = bitmap
                }
            } else {
                val bitmap = BitmapCacheHelper.createBitmap(KEY, filePath, imageSampleSize)
                if (bitmap != null) {
                    imageMap[entry.key] = bitmap
                }
            }
    ...
    private fun setupAudios(entity: MovieEntity, completionBlock: () -> Unit) {
        ...
        // Repair missing callbacks when audioEntity is invalid.
        // If audiosFileMap is empty, soundPool?.load is not called, so setOnLoadCompleteListener
        // will not be invoked and outer prepare would hang.
    ...
    private fun createSvgaAudioEntity(audio: AudioEntity, audiosFileMap: HashMap<String, File>): SVGAAudioEntity {
        ...
        // Return files directly and skip the later playback flow.
        mPlayCallback?.let {
    ...
    fun clear() {
        ...
        imageMap.clear()
        BitmapCacheHelper.removeRelated(KEY)
    }
}
